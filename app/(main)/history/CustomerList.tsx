...
import { fetchCustomers, updateCustomer, deleteCustomer } from "@/lib/api/customers"
...
export default function CustomerList({ initialData }: CustomerListProps) {
  ...
  const { data: customers = initialData, mutate, isLoading } = useSWR(
    'customers',
    () => fetchCustomers(),
    {
  ...
  const handleDelete = async (id: string | undefined) => {
    ...
    const optimisticData = customers.filter(c => c.id !== id)
  ...
  const handleMarkAsVisited = async (c: Customer) => {
    if (!c.id) return
    ...
    // Prepare optimistic data: current customers with the target one updated
    const updatedCustomers = customers.map(cust => 
      cust.id === c.id ? { ...cust, visit_date: now } : cust
    )

    try {
      // Use SWR's mutate with optimisticData
      await mutate(
        (async () => {
          const result = await updateCustomer(c.id!, { visit_date: now })
          return customers.map(cust => cust.id === c.id ? result : cust)
        })(),
        {
          optimisticData: updatedCustomers,
          rollbackOnError: true,
          revalidate: false,
        }
      )
      toast.success("已标记为今日拜访")
    } catch (e) {
      toast.error("操作失败")
      console.error(e)
    }
  ...
  const uniqueIndustries = Array.from(new Set(customers.map(c => c.industry).filter((i): i is string => !!i)))
            ...
            <div className="hidden md:block">
              <DataTable
                ...
                data={filteredCustomers.map(c => [
                  ...
                  <span className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-[var(--glass-bg)] border border-[var(--border)] text-[var(--fg-muted)]" key={c.id}>{c.industry || '未分类'}</span>,
                  <span className="text-sm text-[var(--fg)]" key={c.id}>{c.contact || '-'}</span>,
                  <div className="flex items-center gap-2" key={c.id}>
                    <span className={`w-2 h-2 rounded-full ${(c.intent_level ?? 0) >= 4 ? 'bg-green-500' : c.intent_level === 3 ? 'bg-yellow-500' : 'bg-red-500'}`}></span>
                    <span className="text-sm font-medium text-[var(--fg)]">{getIntentLevelLabel(c.intent_level || 1)}</span>
                  </div>,
                  <span className="text-sm text-[var(--fg-muted)] tabular-nums" key={c.id}>{c.visit_date ? new Date(c.visit_date).toLocaleDateString() : '-'}</span>,
                  <div className="text-right" key={c.id}>
                    <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleMarkAsVisited(c)} className="text-sm font-medium text-green-600 hover:text-green-700 hover:underline">记拜访</button>
                ...
                <div key={c.id} className="p-5 flex flex-col gap-3 hover:bg-[var(--glass-bg)] transition-colors">
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                        (c.intent_level ?? 0) >= 4 ? 'bg-green-500/10 text-green-500' :
                  ...
                  <div className="flex items-center justify-between pt-2 mt-1">
                      ...
                      <div className="flex gap-4">
                          <button onClick={() => handleMarkAsVisited(c)} className="text-sm font-medium text-green-600 hover:text-green-700">
                              记拜访
                          </button>
